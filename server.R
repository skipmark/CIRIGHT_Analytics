...
function(input, output, session) {
  ############################# World Map #################################
  output$world_map<-renderTmap({
    
    #subsetting data according to user input
    maptemp<- filter(mapdata,mapdata$year==input$yearworld)
    
    #creating a world map with tmap
      tmap_mode("view")
      tm_shape(maptemp)+
      tm_polygons("human_rights_score",
                  title= "Human Rights Index",
                  textNA ="",
                  palette="RdYlBu",
                  labels=c("No Respect", "", "", "","", "","","","Full Respect"),
                  midpoint=60,
                  breaks = c(0,20,30,40,50,60,70,80,90,100),
                  id="country",
                  popup.vars= c("Country"="country", "Overall Human Rights"="human_rights_score", "Worker Rights (Out of 36)"="workerrights_practices_sum","Civil & Political Rights (Out of 24)"="civpol_sum","Physical Integrity Rights (Out of 16)"="physint_sum",  "Repression Index (Out of 17)" ="repression_sum")) +
      tm_borders()  +
      tm_layout (frame = FALSE, bg.color = "transparent")
    #world
    
    # #creating a world map with ggplot2
  ...
  output$reg_map<-renderTmap({
    
    ##Africa
    
    if(input$region=="africa"){
      #subsetting data according to user input
      maptemp<- filter(mapdata,mapdata$africa==1 & mapdata$year==input$yearreg)
      #creating a world map with tmap
      tmap_mode("view")
      region<-tm_shape(maptemp)+
        tm_polygons("human_rights_score",
                    title= "Human Rights Index",
                    textNA ="",
                    palette="RdYlBu",
    ...
    if(input$region=="oceania"){
      #subsetting data according to user input
      maptemp<- filter(mapdata,mapdata$year==input$yearreg & mapdata$oceania==1)
    ...
    if(input$region=="americas"){
      #subsetting data according to user input
      maptemp<- filter(mapdata,mapdata$year==input$yearreg & mapdata$americas==1)
    ...
    if(input$region=="asia_middle_east"){
      #subsetting data according to user input
      maptemp<- filter(mapdata,mapdata$year==input$yearreg & mapdata$asia_middle_east==1)
    ...
    if(input$region=="europe"){
      #subsetting data according to user input
      maptemp<- filter(mapdata,mapdata$year==input$yearreg & mapdata$europe==1)
  ...
  output$phint_raw_scores <- renderPlot({
    ...
    ##Summary Index
    if(input$phys_int_type=="physint_sum"){
  ...
  output$emp_raw_scores <-renderPlot({
    ...
    ##Summary Index
    if(input$emp_rights=="emp_rights_sum"){
      #removing missing values
      temp1 <- temp1[!is.na(temp1$emp_rights_sum),]
      #Validating data
      validate(
        need(nrow(temp1)!=0, "Sorry, currently there is no data for this country, but we are working on it :) "
        )
  ...
  ###################Civil and Political Rights Raw score ######################
  output$civpol_raw_scores <-renderPlot({
    ##  Subset out the data that the user selects in the app
    ##
    temp1 <- subset(hrdata,
                    hrdata$country == input$country_civpol & hrdata$year %in% seq(input$countryyearslider_civpol[1],
                                                                               input$countryyearslider_civpol[2], 1))
    #Validating data
    validate(
      need(nrow(temp1)!=0, "Sorry, currently there is no data for this country, but we are working on it :) "
      )
    ...
    ##Summary Index
    if(input$civpol_rights=="civpol_sum"){
      #removing missing values
      ...
      plot <- ggplot(temp1, aes(x=year, y=civpol_sum)) +
        ...
        ylab("Score")+
        theme(axis.title.y = element_text(size=14,face="bold"),axis.title.x = element_text(size=14,face="bold"),axis.text.x = element_text(size=14),axis.text.y = element_text( angle=90, vjust=1, hjust=.5,size=14 ))+
        ggtitle(paste("Civil and Political Rights Index", "\n", "(", paste(input$country_civpol, ":", sep=""), input$countryyearslider_civpol[1], "-",  input$countryyearslider_civpol[2], ")")) }
    
    

    # ##  Freedom of Speech and Press

    if(input$civpol_rights=='speech'){
      #removing missing values
    ...
    ##Freedom of religion
    if(input$civpol_rights=='rel_free'){
      #removing missing values
  ...
  ###################Civil and Political Rights Rolling average score ######################
  
  output$civpol_roll_avg <-renderPlot({
...
print(plot)
  ...
  output$work_raw_scores<-renderPlot({
    ...
    ## Summary Index
    if(input$workers_rights=="workerrights_practices_sum"){
      #removing missing values
    ...
    if(input$workers_rights=="workerrights_laws_sum"){
      ...
      plot <- ggplot(temp1, aes(x=year, y=workerrights_laws_sum)) +
        ...
        ylab("Score")+
        theme(axis.title.y = element_text(size=14,face="bold"),axis.title.x = element_text(size=14,face="bold"),axis.text.x = element_text(size=14),axis.text.y = element_text( angle=90, vjust=1, hjust=.5,size=14 ))+
        ggtitle(paste("Workers' Rights Laws Index", "\n", "(", paste(input$country_work, ":", sep=""), input$countryyearslider_work[1], "-",  input$countryyearslider_work[2], ")")) }
  ...
  output$justice_raw_scores<-renderPlot({
    ...
    if(input$justice_rights=='ngo_freedom'){
      #removing missing values
      temp1 <- temp1[!is.na(temp1$ngo_freedom),]
      #Validating data
      validate(
        need(nrow(temp1)!=0, "Sorry, currently there is no data for this country, but we are working on it :) "
        )
      ...
      # Convert column from numeric to factor variable
      temp1$ngo_freedom <- as.factor(temp1$ngo_freedom)
      #For highlighting different scores
      highlight_df_1<-temp1[temp1$ngo_freedom == "1", ]  
      highlight_df_2<-temp1[temp1$ngo_freedom == "2", ]  
      highlight_df_3<-temp1[temp1$ngo_freedom == "3", ]  
      
      plot <- ggplot(temp1, aes(x=year, y=ngo_freedom)) +
        geom_point(color="red", size=3)+
        geom_point(data=highlight_df_1, aes(x=year,y=ngo_freedom), color='orange',size=3)+
        geom_point(data=highlight_df_2, aes(x=year,y=ngo_freedom), color='green', size=3)+ 
        geom_point(data=highlight_df_3, aes(x=year,y=ngo_freedom), color='darkgreen', size=3)+ 
        xlab("Years") + 
        ylab("Score")+
        scale_y_discrete(labels=c("0" = "Subject\n to Violence (0)" , "1"='Harassed (1)' , "2"="Generally\n Free (2)", "3"="Full\n Freedom (3)"))+ 
        theme(axis.title.y = element_text(size=14,face="bold"),axis.title.x = element_text(size=14,face="bold"),axis.text.x = element_text(size=14),axis.text.y = element_text( angle=90, vjust=1, hjust=.5,size=14 ))+
        ggtitle(paste("Human Rights NGO Freedom Score", "\n", "(", paste(input$country_justice, ":", sep=""), input$countryyearslider_justice[1], "-",  input$countryyearslider_justice[2], ")")) }
  ...
  output$justice_roll_avg<-renderPlot({
    ...
    if(input$justice_rights=='ngo_freedom'){
      #removing missing values
      temp1 <- temp1[!is.na(temp1$avg_ngo3),]
      validate(
        need(nrow(temp1)!=0, "Sorry, currently there is no data for this country, but we are working on it :) "
        )
      )
      #rolling average plot
      plot <- ggplot(temp1, aes(x=year, y=avg_ngo3)) +
        geom_line()+
        xlab("Years") + 
        ylab("Rolling Average")+ 
        theme(axis.title.y = element_text(size=14,face="bold"),axis.title.x = element_text(size=14,face="bold"),axis.text.x = element_text(size=14),axis.text.y = element_text( angle=90, vjust=1, hjust=.5,size=14 ))+
        ggtitle(paste("Rolling 3 year Average of The Human Rights NGO Freedom Score", "\n", "(", paste(input$country_justice, ":", sep=""), input$countryyearslider_justice[1], "-",  input$countryyearslider_justice[2], ")")) 
    }
  ...
  )#empofwerment rights download ends here
  
  #Civil and Political Rights
  output$ downloadCivPolData <- downloadHandler(
    filename = function() { paste(input$country_civpol, "_", 
                                  input$countryyearslider_civpol[1], "-", 
                                  input$countryyearslider_civpol[2], 
                                  '.csv', 
                                  sep='') },
    content = function(file) {
      write.csv(subset(hrdata, hrdata$country == input$country_civpol), file)
    }
  )#empofwerment rights download ends here
