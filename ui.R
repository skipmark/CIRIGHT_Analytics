...
navbarPage("CIRIGHTS Analytics",
           tabPanel("World Trends",
                    plotlyOutput("world_map",height = '150%', width = '100%'),
                    hr(),
                    fluidRow(
                      column(4, offset=5,
                            sliderInput("yearworld", "Choose year",min=1981, max=2020,value=1,
           ...
           tabPanel("Regional Trends",
                    plotlyOutput("reg_map",height = '150%', width = '100%'),
                    hr(),
                    fluidRow(
                      column(4, offset=5,
                             selectInput("region", "Choose a region:",
                                                         c("Africa" = "2",
                                                           ...
                                                           "Asia" = '142',
                                                           "Europe"="150",
                                                           "Latin America"="419")), #selectInput ends
                             sliderInput("yearreg", "Choose year",min=1981, max=2020,value=1,
                                         sep="", animate=TRUE),#sliderInput ends
                    ...
                    sidebarLayout(
                      sidebarPanel(##Select which country you want
                        ...
                        selectInput("phys_int_type", "Choose a category of Physical integrity rights:",
                                    c("Political/Extrajudicial Killings" = 'kill',
                                      "Disappearance" = 'disap',
                    ...
                    sidebarLayout(
                      sidebarPanel(##Select which country you want
                        ...
                        selectInput("emp_rights", "Choose a category of Empowerment rights:",
                                    c("Freedom of Speech and Press" = 'speech',
                    ...
                    sidebarLayout(
                      sidebarPanel(##Select which country you want
                        ...
                        selectInput("workers_rights", "Choose a category of Workers' rights:",
                                    c("The Right to Form Worker Unions (Law)" = 'union_l',
      ...
      tabPanel("Summary Scores",
               sidebarLayout(
                 sidebarPanel(##Select which country you want
                   selectInput("country_summary", "Choose country:",
                               choices=as.character(sort(unique(hrdata$country))),
                               selected="Afghanistan"), #selectInput ends
                   #select a type of justice rights you want
                   selectInput("summary_rights", "Choose a category of Justice rights:",
                               c("Physical Integrity Rights"="physint_sum",
                                 "Repression Index"="repression_sum",
                                 "Civil and Political Rights Index"="civpol_sum",
                                 "Worker Rights Laws"="workerrights_laws_sum",
                                 "Worker Rights Practices"="workerrights_practices_sum"
                                 
                               )), #selectInput ends
                   sliderInput("countryyearslider_summary", "Choose year",
                               min=1981, max=2020,
                               value=c(1981, 2020),
                               sep=""),#sliderInput ends
                   downloadButton('downloadSummaryData', "Download these data"),
                   helpText("Note: Downloads user-selected country and year range.")
                 ),#sidebarPanel ends
                 #Create a spoty for line plot
                 mainPanel(
                   tabsetPanel(
                     tabPanel("Raw Country Scores",
                              plotOutput("summary_scores")) #raw scores for each year
                     ), #demands ends
                   )#tabsetPanel ends
                 )#mainPanel ends
               ) #sidebarLayout ends
